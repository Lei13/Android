...
import com.android.prictice.json.util.DiskLruCache.Snapshot;

/**
 * 解决 类似 ListView 与 GridView 等视图组件在使用AsyncTask异步加载图片滚动时出现图片错乱问题，
 * AsyncTask异步多线程技术 内存缓存实现LRUCache 文件缓存DiskCache
 * 
 * @author Viktor.zhou
 * 
 */
public class AsyncMemoryFileCacheImageLoader {
	...
	public void loadBitmap(Resources res, String imageUrl, ImageView imageView,
		...
		if (cancelPotentialWork(imageUrl, imageView)) {
			...
			if (mExec == null) {
				task.execute(imageUrl, String.valueOf(width),
	 ...
	 * @param imageUrl
	 * @param imageView
	 *            widht height 默认为 0 R.drawable.transparent：
	 *            在res/values文件夹下定义drawables.xml文件，设置如下值：表示默认图片透明 <item
	 *            name="transparent" type="drawable">#00000000</item>
	 */
	public void loadBitmap(Resources res, String imageUrl, ImageView imageView) {
		Bitmap bitmap = getBitmapFromMemoryCache(imageUrl);
		if (bitmap != null) {
		...
		if (cancelPotentialWork(imageUrl, imageView)) {
			...
			if (mExec == null) {
				...
				task.executeOnExecutor(mExec, imageUrl, String.valueOf(0),
						String.valueOf(0));
			}
	...
	public static int calculateInSampleSize(BitmapFactory.Options options,
		...
		// Logs.v("calculateInSampleSize  height :" + height + " width :" +
		// width);
		int inSampleSize = 1;
	 ...
	 * 方法来获取缓存路径。 前者获取到的就是 /sdcard/Android/data/<application package>/cache这个路径，
